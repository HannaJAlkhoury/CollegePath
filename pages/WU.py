...
st.set_page_config(page_title="College Path- جامعة الوادي", page_icon=icon, layout= "wide")
def cost(unicost):
...
with st.sidebar:
    ...
    select = option_menu(
        menu_icon="bank2",
        menu_title="جامعة الوادي الدولية",
...
if select=="على الخريطة":
    ...
    st.write("###")
    gmap="""<p><iframe src="https://www.google.com/maps/d/embed?mid=1ZKfi_O11E6EpVtsxIGaUR70YPsSVo70&ehbc=2E312F" width="100%" height="600" style="border:10;" allowfullscreen="True" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></p>"""
    st.markdown(gmap, unsafe_allow_html=True)
if select=="معلومات عن الجامعة":
    st.write("###")
    st.markdown("<h2 style='text-align: right; color: #00B0F0;'> جامعة الوادي الدولية </h2>" , unsafe_allow_html=True)
    st.write("###")
    ...
    if selectuni=="نظرة عامة":
        l,r= st.columns(2)
        with l:
            image=Image.open("pictures/WU.jpg")
            st.image(image)
        with r:
            st.write("<a href='http://www.wiu.edu.sy/' style='text-align: right; color: #0070C0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'> الموقع الإلكتروني🌐 </a>", unsafe_allow_html=True)
            st.write("<a href='https://www.google.com/maps/search/%D8%AC%D8%A7%D9%85%D8%B9%D8%A9+%D8%A7%D9%84%D9%88%D8%A7%D8%AF%D9%8A%E2%80%AD/@34.7113416,36.2929796,18z?authuser=0&entry=ttu' style='text-align: right; color: #0070C0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'> الموقع على الخريطة🗺  </a>", unsafe_allow_html=True)
            st.markdown("<h5 style='text-align: right; color: #000000;'> نوع الجامعة : خاصة </h5>" , unsafe_allow_html=True)
            st.markdown("<h5 style='text-align: right; color: #000000;'> ترتيب الجامعة على سوريا حسب ويبوميتريكس : 15 </h5>" , unsafe_allow_html=True)
        st.write("---")
        st.markdown("<h2 style='text-align: right; color: #00B0F0;'> نبذة عن الجامعة </h2>" , unsafe_allow_html=True)
        st.markdown("<h5 style='text-align: right; color: #000000;'> جامعة الوادي الدولية الخاصة هي جامعة خاصة تأسست عام 2005 وتقع في منطقة وادي النضارة (وادي النصارى) </h5>" , unsafe_allow_html=True)  
        st.write("---")
        st.markdown("<h2 style='text-align: right; color: #00B0F0;'> صفحات الجامعة على مواقع التواصل </h2>" , unsafe_allow_html=True)
        st.write("<a href='https://www.facebook.com/WiuUniversity/' style='text-align: right; color: #0030f0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  Facebook-فيسبوك 🟦 </a>", unsafe_allow_html=True)
        st.write("<a href='https://www.instagram.com/wadi_university/' style='text-align: right; color: #d0d010; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  Instagram-إنستاغرام 🟨 </a>", unsafe_allow_html=True)
        st.write("<a href='https://www.linkedin.com/company/wadi-internationa-university/' style='text-align: right; color: #0090e0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  LinkedIn-لينكدإن 🔷 </a>", unsafe_allow_html=True)
    if selectuni=="الرسوم الدراسية":
        ...
        for loop in range(38):
            ...
            allfees.append(l)
        list=[]
        majors=["الطب البشري","طب الأسنان","الصيدلة","الهندسة المدنية","الهندسة المعلوماتية","الحقوق","العلوم الإدارية والاقتصادية","هندسة العمارة"]
    ...
    if selectuni=="كليات الجامعة":
        ...
        """
        st.markdown("<h4 style='text-align: right; color: #000000;'> كلية الهندسة بأقسام العمارة والمدنية والمعلوماتية </h4>" , unsafe_allow_html=True)
        ...
        st.markdown("<h4 style='text-align: right; color: #000000;'> كلية الحقوح </h4>" , unsafe_allow_html=True) 
        st.markdown("<h4 style='text-align: right; color: #000000;'> كلية العلوم الإدارية والاقتصادية </h4>" , unsafe_allow_html=True) 
        st.markdown("<h4 style='text-align: right; color: #000000;'> كلية الطب </h4>" , unsafe_allow_html=True)   