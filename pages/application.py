...
.st-emotion-cache-9hz7ad p, .st-emotion-cache-9hz7ad ol, .st-emotion-cache-9hz7ad ul, .st-emotion-cache-9hz7ad dl, .st-emotion-cache-9hz7ad li{{text-align: right;}}"""
...
with st.sidebar:
     ...
     st.write("###")
st.markdown("<h3 style='text-align: right; color: #00B0F0; font-weight:bold;'> ما هي المفاضلة؟ </h3>", unsafe_allow_html=True)
...
st.write("---")   
st.markdown("<h3 style='text-align: right; color: #00B0F0; font-weight:bold;'> قبل التقدم على المفاضلة </h3>" , unsafe_allow_html=True)
...
st.markdown("<h5 style='text-align: right; color: #000000;'> تحديد الجامعات التي تناسب مكان سكن الطالب وإمكانياته المادية والاختصاصات التي يرغبها وهنا يمكنه الاستعانة بمحرك البحث عن الأفرع في الموقع لإيجاد الجامعات التي تقدم الاختصاصات المناسبة له  </h5>" , unsafe_allow_html=True)
st.markdown("<h4 style='text-align: right; color: #0070C0;'> ثالثاً :</h4>" , unsafe_allow_html=True)
st.markdown("<h5 style='text-align: right; color: #000000;'> إعداد قائمة بالأفرع المراد التقديم عليها تحوي اسم الاختصاص والجامعة بالترتيب من الأكثر تحبيذاً للأقل تحبيذاً و على الطالب أيضاً اختيار بعض البدائل في حال عدم قبوله في الأفرع المرادة ونحن ننصح بالإكثار من البدائل وذلك خوفاً على الطالب من أن ترفض جميع رغباته التي قدمها في المفاضلة بسبب ارتفاع علامات القبول في الكثير من الأفرع بين حد العلامات الذي يؤهل الطالب للتقديم على الفرع (ويسمى المفاضلة الأولى) والحد الذي يقبل به الطلاب (ويسمى المفاضلة الثانية أو النتائج)  </h5>", unsafe_allow_html=True)
st.markdown("<h4 style='text-align: right; color: #0070C0;'> رابعاً :</h4>" , unsafe_allow_html=True)
st.markdown("<h5 style='text-align: right; color: #000000;'> فور الإعلان عن بدء المفاضلة على الطالب إيجاد أكواد التقديم لكل فرع اختاره والكود هو رقم خاص بكل اختصاص في كل جامعة وهو الذي يتم إدخاله إلى موقع المفاضلة للتقديم عليها ويتم الحصول عليه من ملف تصدره الوزارة كل سنة </h5>", unsafe_allow_html=True)
st.markdown("<h5 style='text-align: right; color: #00B0F0;'> بعض الأفرع لبعض الجامعات لا تحوي أكواد للمفاضلة مثل أفرع الجامعة الافتراضية أو الأفرع الغير طبية للجامعات الخاصة لأن تلك الأفرع لديها المفاضلة الخاصة بها (اقرأ الفقرة التالية للمزيد من المعلومات) </h5>", unsafe_allow_html=True)
st.write("---")
st.markdown("<h3 style='text-align: right; color: #00B0F0; font-weight:bold;'> بعض المصطلحات الهامة التي يجب فهمها :</h3>", unsafe_allow_html=True)
st.write("###")
left, right = st.columns(2)
with left:
     imgprint("images/photo.webp") 
with right:
     dropbox="text-align: right; width: 100%; display: inline-flex;-webkit-box-align: center;align-items: center;-webkit-box-pack: center;justify-content: flex-end; font-weight: bolder;padding: 0.25rem 0.75rem;border-radius: 0rem;min-height: 38.4px;margin: 0px;line-height: 1.6;width: 100%;user-select: none;background-color: #f0f9ff;color: #000000;border: 1px solid rgb(0 176 240 / 0%);align-content: center;border-bottom-color:#00b0f0; font-size:1.2rem"   
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> السنة التحضيرية </summary><p style='font-weight:bold;font-size:1.1rem'> على الراغبين بدراسة أحد الاختصاصات الطبية (الطب البشري، طب الأسنان أو الصيدلة) في جامعة حكومية التقديم على السنة التحضيري حيث يدخل معدلهم في هذه السنة مع معدل البكلوريا في فرزهم على الاختصاصات المختلفة </p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> الافتراضي </summary><p style='font-weight:bold;font-size:1.1rem'><h style='text-align: right;'> هو الدراسة عبر الجامعة الافتراضية السورية حيث يدرس الطالب فيها كلياً عبر الانترنت حيث يحق للطالب أن يسجل ويدرس في الجامعة الافتراضية بالإضافة إلى أي فرع آخر في أي جامعة (أي يمكن للطالب أن يدرس الهندسة المعلوماتية بالجامعة الافتراضية بالإضافة إلى دراسة الرياضيات في جامعة دمشق) </h><h style='text-align: right; color: #00B0F0;'>: يتم التقديم على الجامعة الافتراضية عبر موقعها الرسمي </h><a href='#' id='https://svuonline.org/ar' style='font-weight:bold;font-size:1.1rem'>https://svuonline.org/ar</a></p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> التعليم المفتوح </summary><p style='font-weight:bold;font-size:1.1rem'> تتيح الجامعات الحكومية إمكانية دراسة تخصصات متعددة عبر التعليم المفتوح حيث يمكن للطالب دراسة فرع في التعليم المفتوح بالإضافة لفرعه الرئيسي، من التخصصات المتاحة الإعلام، الدراسات القانونية، إدارة المشروعات الصغيرة والمتوسطة،المحاسبة، الترجمة والتربية </p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> المعاهد العليا والتقانية </summary><p style='font-weight:bold;font-size:1.1rem'><h style='text-align: right;'> المعاهد العليا شبيهة بالكليات من حيث سنوات الدراسة والإجازات الممنوحة، أما المعاهد التقانية فتكون سنوات الدراسة فيها سنتان والهدف منها هو تخريج خبراء في مجالات تطبيقية في مختلف المجالات التي يتطلبها سوق العمل </h><h style='text-align: right; color: #00B0F0;'> الطلاب المتفوقون في المعاهد التقانية قد يحق لهم الالتحاق ببعض الاختصاصات الجامعية  الموافقة بعد تخرجهم من المعهد </h></p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> اختبارات القبول </summary><p style='font-weight:bold;font-size:1.1rem'> يتطلب التقديم على بعض الاختصاصات ومن أهمها العمارة واللغة الإنجليزية والفرنسية وبعض كليات الفنون وبعض المعاهد النجاح باختبارات اضافية، يرجى قراءة ملف البي دي إف لمعرفة التفاصيل <a href='#' download='exams.pdf' style='font-weight:bold;font-size:1.1rem'> اضغط لتنزيل الملف </a></p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> الطلب العام </summary><p style='font-weight:bold;font-size:1.1rem'> يقدم الطالب على أحد الفروع في إحدى الجامعة الحكومية للدراسة  <h style='color: #00B0F0;'>مجاناً</h></p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> الطلب الموازي </summary><p style='font-weight:bold;font-size:1.1rem'>  يقدم الطالب على أحد الفروع في جامعة حكومية للدراسة مقابل مبلغ مالي تتميز بحد أدنا للعلامات أقل بقليل من العام فتسمح للطالب الذي لم تمكنه علاماته من أن يقبل طلبه العام أن يدخل الفرع مقابل مبلغ مالي ضئيل نسبة لأقساط الجامعات الخاصة و لا تتغير أقساط التعليم الموازي على الطالب نفسه من سنة إلى أخرى بل تبقى ثابتة وتساوي الأقساط التي تم تحديدها له في السنة التي سجل بها </p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> الطلب الخاص </summary><p style='font-weight:bold;font-size:1.1rem'> يقدم الطالب على أحد الفروع في جامعة خاصة ويترتب عليه رسوم دراسية تفرضها الجامعة، يتم التقديم على الفروع الطبية من خلال مفاضلة التعليم العالي أما التقديم على باقي الاختصاصات يتم عبر مفاضلات تحددها كل جامعة  <h style='color: #00B0F0;'>، لا تتغير أقساط التعليم الخاص على الطالب من سنة إلى أخرى بل تبقى ثابتة وتساوي الأقساط التي تم تحديدها له في السنة التي سجل بها </h></p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> منح الجامعات الخاصة </summary><p style='font-weight:bold;font-size:1.1rem'>  يقدم الطالب عبر مفاضلة وزارة التعليم العالي حصراً على منحة لدراسة أحد الفروع في جامعة خاصة مجاناً دون رسوم دراسية وتتكفل المنحة بأجور النقل أو السكن الجامعي أيضاً ويفضل التواصل مع كل جامعة لمعرفة كافة تفاصيل المنحة ومميزاتها لكن لهذا العام لم يتم إعطاء أي مقاعد للمنح،  <h style='color: #00B0F0;'> ملاحظة: لا يحق للطالب في حال قبوله في أحد منح الجامعات الخاصة الاستنكاف عنه او تغيير رغبت </h></p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> ملء الشواغر </summary><p style='font-weight:bold;font-size:1.1rem'> في الكثير من الأحيان قد يتوفر في الجامعات الخاصة بعض المقاعد الشاغرة بعد انتهاء المفاضلة عليها لذلك يتم اللجوء لمفاضلة ملء الشواغر حيث يتوفر بعض المقاعد ويكون الحد الأدنى للقبول أقل بقليل من ذلك للطلب الخاص ولا تختلف أبداً عن الطلب الخاص بأي شيء آخر إلا أن الطالب قد يضطر لدخول الجامعة في منتصف الفصل الدراسي أو يتم تأجيله فصلا كاملاً لذلك ينصح بالتقديم في المفاضلة الخاصة أولاً وعدم الانتظار لمفاضلة ملء الشواغر إلا عند رفض الطلب الخاص للفرع المطلوب </p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> مفاضلة المحافظات الشرقية </summary><p style='font-weight:bold;font-size:1.1rem'> يستطيع أن يتقدم إليها جميع الطلاب السوريين ومن في حكمهم الحاصلين على الشهادة الثانوية العامة السورية من إحدى محافظات دير الزور – الحسكة –الرقة،  ويختار الطالب الجامعة التي يريد أن يداوم فيها ويعامل معاملة طلاب تلك الجامعة دون تفرقة  <h style='color: #00B0F0;'>  نحن ننصح أبناء المحافظات الشرقية بإضافة طلبات المحافظات الشرقية إلى مفاضلتهم لأنها تزيد من فرص قبولهم </h></p></details>",unsafe_allow_html=True) 
     st.markdown(f"<details style='text-align: right;'><summary style='{dropbox}'> مفاضلة أبناء أعضاء الهيئة التدريسية </summary><p style='font-weight:bold;font-size:1.1rem'>  يمكن أن يتقدم عليها أبناء أعضاء هيئة التدريس الحاليين في الجامعات الحكومية، وأبناء أعضاء هيئة البحث العلمي في مركز الدراسات والبحوث العلمية، وأبناء أعضاء هيئة التدريس الحاليين أو السابقين في المعاهد العليا التابعة لوزير التعليم العالي الذين أمضوا عشر سنوات على الأقل في هيئة التدريس أو البحث العلمي من ترك العمل لأسباب غير تأديبية، أو توفي أثناء الخدمة ، تتميز هذه بأنه تتيح للطلاب المستفيدين القبول في الجامعات الحكومية بمعدلات أقل بكثير من أقرانهم لكن يترتب عليهم الحصول على وثيقة تثبت أنهم يحققون شروط التقدم على  هذه المفاضلة وأخذها معهم عند التقديم ويجب أن يكون لم يمض على تاريخ منح الوثيقة شهر واحد ويحصل عليها من ذاتية الجامعة أو المعهد <h style='color: #00B0F0;'> يتم التقديم على هذه المفاضة عبر مراكز تقديم خاصة وليس عن بعد عبر الموقع كما باقي المفاضلات </h></p></details>",unsafe_allow_html=True)                
st.write("---") 
...
st.markdown("<h5 style='text-align: right; color: #000000;'> يتم الدفع الإلكتروني عبر حساب شام كاش </h5>", unsafe_allow_html=True)
st.markdown("<h4 style='text-align: right; color: #0070C0;'>> للطلاب السوريين الغير مقيمين </h4>" , unsafe_allow_html=True)
st.markdown("<h5 style='text-align: right; color: #f03030;'> يتقدم الطالب على المفاضلة إلكترونياً </h5>" , unsafe_allow_html=True)
