...
st.set_page_config(page_title="College Path- جامعة أنطاكيا", page_icon=icon, layout= "wide")
def cost(unicost):
...
if select=="على الخريطة":
    ...
    st.write("###")
    gmap="""<p><iframe src="https://www.google.com/maps/d/embed?mid=1cE0U0mOmFCYIJysoHY45mmKKPLIH4O0&ehbc=2E312F" width="100%" height="600" style="border:10;" allowfullscreen="True" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></p>"""
    st.markdown(gmap, unsafe_allow_html=True)
if select=="معلومات عن الجامعة":
    st.write("###")
    st.markdown("<h2 style='text-align: right; color: #00B0F0;'>جامعة أنطاكيا السورية الخاصة</h2>" , unsafe_allow_html=True)
    st.write("###")
    ...
    if selectuni=="نظرة عامة":
        l,r= st.columns(2)
        with l:
            image=Image.open("pictures/ANTU.jpg")
            st.image(image)
        with r:
            st.write("<a href='https://asu.edu.sy/' style='text-align: right; color: #0070C0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'> الموقع الإلكتروني🌐 </a>", unsafe_allow_html=True)
            st.write("<a href='https://www.google.com/maps/place/%D8%AC%D8%A7%D9%85%D8%B9%D8%A9+%D8%A3%D9%86%D8%B7%D8%A7%D9%83%D9%8A%D8%A7+%D8%A7%D9%84%D8%B3%D9%88%D8%B1%D9%8A%D8%A9%E2%80%AD/@33.6718996,36.3794087,16.25z/data=!4m6!3m5!1s0x1518ed5762ba9f2b:0xfc042375a13369d1!8m2!3d33.6726588!4d36.379614!16s%2Fg%2F11hf6n1qyx?authuser=0&entry=ttu' style='text-align: right; color: #0070C0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'> الموقع على الخريطة🗺  </a>", unsafe_allow_html=True)
            st.markdown("<h5 style='text-align: right; color: #000000;'> نوع الجامعة : خاصة </h5>" , unsafe_allow_html=True)
            st.markdown("<h5 style='text-align: right; color: #000000;'> ترتيب الجامعة على سوريا حسب ويبوميتريكس : 22 </h5>" , unsafe_allow_html=True)
        st.write("---")
        st.markdown("<h2 style='text-align: right; color: #00B0F0;'> نبذة عن الجامعة </h2>" , unsafe_allow_html=True)
        st.markdown("<h5 style='text-align: right; color: #000000;'> جامعة أنطاكية السورية الخاصة، هي جامعة سورية خاصة تأسست بموجب المرسوم رقم /233/ لعام 2017، وتمّ الافتتاح في أيلول 2018 </h5>" , unsafe_allow_html=True)  
        st.write("---")
        st.markdown("<h2 style='text-align: right; color: #00B0F0;'> صفحات الجامعة على مواقع التواصل </h2>" , unsafe_allow_html=True)
        st.write("<a href='https://www.facebook.com/Antioch.Syrian.University/' style='text-align: right; color: #0030f0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  Facebook-فيسبوك 🟦 </a>", unsafe_allow_html=True)
...
<style>
        ...
        for loop in range(28):
            ...
            allfees.append(l)
        list=[]
        majors=["طب الأسنان","الصيدلة","الهندسة المدنية","الحقوق","هندسة العمارة","هندسة الحاسب"]
        ...
        st.markdown("<h5 style='text-align: right; color: #000000;'> بين 300 و800 ألف ليرة سورية شهرياً باستخدام شركات نقل خاصة </h5>" , unsafe_allow_html=True)
    ...
    if selectuni=="المنشآت التابعة":
        ...
        st.markdown("<h5 style='text-align: right; color: #000000;'>لا تقدم الجامعة خدمة السكن الجامعي</h5>" , unsafe_allow_html=True)
    if selectuni=="كليات الجامعة": 
        ...
        st.markdown("<h5 style='text-align: right; color: #000000;'> كلية العلوم الأساسية </h5>" , unsafe_allow_html=True) 
        st.markdown("<h5 style='text-align: right; color: #000000;'> كلية الهندسة </h5>" , unsafe_allow_html=True) 
        st.markdown("<h5 style='text-align: right; color: #000000;'> كلية الآداب والعلوم الإنسانية </h5>" , unsafe_allow_html=True)
        st.markdown("<h5 style='text-align: right; color: #000000;'> كلية العلوم الإدارية والاقتصادية </h5>" , unsafe_allow_html=True) 
