...
st.set_page_config(page_title="College Path- IUST", page_icon=icon, layout= "wide")
def cost(unicost):
...
if select=="على الخريطة":
    ...
    st.write("###")
    gmap="""<p><iframe src="https://www.google.com/maps/d/embed?mid=17THB7Mhm8762Fdycn4SIttmdo9jCFqM&ehbc=2E312F" width="100%" height="600" style="border:10;" allowfullscreen="True" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></p>"""
    st.markdown(gmap, unsafe_allow_html=True)
if select=="معلومات عن الجامعة":
    st.write("###")
    st.markdown("<h2 style='text-align: right; color: #00B0F0;'> الجامعة الدولية الخاصة للعلوم والتكنولوجيا </h2>" , unsafe_allow_html=True)
    st.write("###")
    ...
    if selectuni=="نظرة عامة":
        l,r= st.columns(2)
        with l:
            image=Image.open("pictures/IUST.jpg")
            st.image(image)
        with r:
            st.write("<a href='https://iust.edu.sy/ar/' style='text-align: right; color: #0070C0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'> الموقع الإلكتروني🌐 </a>", unsafe_allow_html=True)
            st.write("<a href='https://www.google.com/maps/place/International+University+for+Science+and+Technology/@33.1827825,36.296339,17z/data=!3m1!4b1!4m6!3m5!1s0x151915cf4d42d5b7:0xf1ab4143108c25c5!8m2!3d33.1827825!4d36.296339!16zL20vMGduMzVj?entry=ttu' style='text-align: right; color: #0070C0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'> الموقع على الخريطة🗺  </a>", unsafe_allow_html=True)
            st.markdown("<h5 style='text-align: right; color: #000000;'> نوع الجامعة : خاصة </h5>" , unsafe_allow_html=True)
            st.markdown("<h5 style='text-align: right; color: #000000;'> ترتيب الجامعة على سوريا حسب ويبوميتريكس : 9 </h5>" , unsafe_allow_html=True)
        st.write("---")
        st.markdown("<h2 style='text-align: right; color: #00B0F0;'> نبذة عن الجامعة </h2>" , unsafe_allow_html=True)
        st.markdown("<h5 style='text-align: right; color: #000000;'> الجامعة الدولية الخاصة للعلوم والتكنولوجيا، جامعة خاصة تأسست عام 2005، وتقع في مدينة غباغب على اوتوستراد دمشق درعا، سوريا. تضم الجامعة ست كليات وهي طب الأسنان والصيدلة والهندسة والتكنولوجيا وتكنولوجيا المعلومات وإدارة الأعمال والتمويل والآداب والعلوم </h5>" , unsafe_allow_html=True)  
        st.write("---")
        st.markdown("<h2 style='text-align: right; color: #00B0F0;'> صفحات الجامعة على مواقع التواصل </h2>" , unsafe_allow_html=True)
        st.write("<a href='https://www.linkedin.com/company/international-university-for-science-and-technology/' style='text-align: right; color: #0010d0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  LinkedIN-لينكدإن 🔵 </a>", unsafe_allow_html=True)
        st.write("<a href='https://www.youtube.com/channel/UCNX0fbbJW0W-bcOTUu-6Fhw' style='text-align: right; color: #f02020; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  YouTube-يوتيوب 🟥 </a>", unsafe_allow_html=True)
        st.write("<a href='https://www.facebook.com/IUST-Official-1672812356291463/' style='text-align: right; color: #0030f0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  Facebook-فيسبوك 🟦 </a>", unsafe_allow_html=True)
        st.write("<a href='https://www.instagram.com/international_university_iust/?hl=en' style='text-align: right; color: #d0d010; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  Instagram-إنستاغرام 🟨 </a>", unsafe_allow_html=True)
        st.write("<a href='https://t.me/iust2022' style='text-align: right; color: #0090e0; font-weight: bolder; font-weight: 600; font-size: 1.2rem;'>  Telegram-تيليغرام 🔷 </a>", unsafe_allow_html=True)
    if selectuni=="الرسوم الدراسية":
        ...
        for loop in range(35):
            ...
            allfees.append(l)
        list=[]
        majors=["طب الأسنان","الصيدلة","هندسة الإلكترونيات والاتصالات","الهندسة المدنية","الهندسة المعلوماتية","العلوم المالية والمصرفية","المحاسبة","إدارة الأعمال","هندسة العمارة","التسويق","اللغة الإنكليزية","التصميم الداخلي","التصميم الغرافيكي"]
        ...
        st.markdown("<h5 style='text-align: right; color: #000000;'> بين 360 و960 ألف ليرة سورية شهرياً باستخدام شركات نقل خاصة حسب البعد عن الجامعة </h5>" , unsafe_allow_html=True)
        st.markdown("<h5 style='text-align: right; color: #000000;'> بحدود 1.26 مليون ليرة سورية شهرياً باستخدام نقل الجامعة </h5>" , unsafe_allow_html=True)    
    if selectuni=="المنشآت التابعة":
        ...
        with rrc:    
            ...
            st.markdown("<h5 style='text-align: right; color: #000000;'>  لا تمتلك الجامعة مستشفى جامعي </h5>" , unsafe_allow_html=True)
    if selectuni=="كليات الجامعة":
        ...
        st.markdown("<h5 style='text-align: right; color: #000000;'> كلية الهندسة والتكنولوجيا </h5>" , unsafe_allow_html=True) 
        st.markdown("<h5 style='text-align: right; color: #000000;'> كلية الهندسة المعمارية </h5>" , unsafe_allow_html=True) 
        st.markdown("<h5 style='text-align: right; color: #000000;'> كلية إدارة الأعمال والتمويل </h5>" , unsafe_allow_html=True) 
        st.markdown("<h5 style='text-align: right; color: #000000;'> كلية الآداب والعلوم </h5>" , unsafe_allow_html=True)   