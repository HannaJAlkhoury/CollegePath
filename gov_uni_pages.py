...
def Uni_Page(uniname,nickname,map_link,uni_image,website,location,rank,description,note,social_media,colleges=[],unihospitals=[],High_inst=[],Mid_inst=[],campus_housing=True):
    ...
    if select=="معلومات عن الجامعة":
        ...
        if selectuni=="نظرة عامة":
            ...
            st.write("---")
            st.markdown("<h3 style='text-align: right; color: #00B0F0;'> نبذة عن الجامعة </h3>" , unsafe_allow_html=True)
            ...
            st.write("---")
            st.markdown("<h3 style='text-align: right; color: #00B0F0;'> صفحات الجامعة على مواقع التواصل </h3>" , unsafe_allow_html=True)
        ...
        if selectuni=="المنشآت التابعة":
            ...
            with llc:
                ...
                if campus_housing:
                    st.markdown("<h5 style='text-align: right; color: #000000;'> تكلفة السكن الجامعي(2024): 80ألف ليرة سورية سنوياً  </h5>" , unsafe_allow_html=True)
                    ...
                    st.markdown("<h5 style='text-align: right; color: #000000;'> المشاكل التي يعاني منها السكن: عدد كبير من الطلاب في الغرفة الواحدة ،حجم الغرف صغير ومستويات النظافة منخفضة </h5>" , unsafe_allow_html=True)
                else:
